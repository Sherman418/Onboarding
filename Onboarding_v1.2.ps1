...
<#
    ...
    If it does exist, $userExist has a value of TRUE and initiates a loop.
    The loop adds a "1" to the end of $userName.
    It then performs a second check to see if $userName + 1 exists in AD.
    If that name is still taken, the number is taken off the end of $userName, and the loop repeats with "2" appended.
#>
$userSuffix = 1
...
while ($userExists -eq $true) {
    ...
    if ($userExists -eq $true) {
        $userName = $userName -replace ".$" #This removes the last character of $userName (the number.) Otherwise you end up with "$userName" + 12 instead of "$userName" + 2.
    }
}
